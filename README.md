...
| 階段 | 里程碑 | 預計完成 | 狀態 | 進度更新 |
|------|--------|----------|------|------|
| Phase 1 | 需求確認 & 架構設計 | Week 1-2 | 🟡 進行中 | 定調主要資料表、API、前後端基本設計 |
| Phase 2 | 前台頁面開發 | Week 3-5 | 🟡 進行中 | 前台頁面開發（已完成React專案初始化，可逐步開發UI功能） |
| Phase 3 | 後台頁面開發（含師資端） | Week 6-9 | 🟡 進行中 | 後台會員面開發（已備妥後端API環境、可隨時進行擴充） |
| Phase 4 | API 與資料庫設計 | Week 5-10 | 🟡 進行中 | 基礎API、資料庫結構已測通（註冊/登入/WorkBench驗證） |
